...
// We default to fixed drive balance being on as that's what we did in
// Washington State regionals.
...
// slower than this we risk the motors (level to be verified)
#    define DRIVE_POWER_STALL  10
#endif
...
// If asked to drive too short of a distance, we don't even bother, as
// we really can't do that well. We also stop the drive when we're
// within this distance of our target.
...
// Balance the power to the left and right wheels according to the
// indicated factor, but always by *reducing* the power to one wheel or
// the other. Note that powerLeft and powerRight both must be variables,
// and both are IN OUT.
...
CALLEDBY(iTaskMain)
  RampDrivePower(DRIVESTATE &state, float fraction) {
    ...
    // Adjust the power to the left and right according to a balance
    // setting which accounts for any intrinsic lopsidedness in the
    // driving of the bot.
    ...
    if (Abs(cmDistance) >= CM_DRIVE_TOLERANCE) {
        ...
        for (;;) {
            ...
            if (CheckStopConditions(stop)) {
                ...
                // We're asked to go an additional distance now that
                // we've stopped.  Implement that using by emulating a
                // recursive call with a goto back to the start of the
                // function.
                // NB: this 'feature' probably wasn't really worth the
                // trouble, but hey.
            ...
            // How much more distance do we have to go?
            // NB: we (somewhat arbitrarily) only look at the LEFT
            // encoder to measure the distance. Idea: might it be better
            // to AVERAGE the two encoders? That's not yet explored, as
            // we're not seeing any issues attributable to the left-only
            // approach.
...
// Drive forwards until the (front) ultrasonic sensor reads the
// indicated value, then a little extra
BOOL
DriveForwardsToSonic(int cmSonic, float cmExtra) {
    ...
    for (;;) {
        ...
        // 20 is arbitrary here, and hasn't really be tested well
        if (!DriveForwards(20))
            return false;
    }
