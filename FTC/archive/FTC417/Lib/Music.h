...
void
Beep(int note = NOTE_A, int msDuration = 10)
...
// Play the indicated music, consisting of cNote pairs of (tone, duration), at the indicated tempo
{
    ...
    for (int iNote = 0; iNote < cNote; iNote++) {
        ...
        //
        int ms =
            (int)((mintesPerBeatWorthOfTicks * (long)ctickDuration) /
            (long)beatsPerMinute);
...
void
PlaySad() {
    PlayMusic(musicSad, 400, NOTE_QUARTER);
}                               // we call this often, so fn is much smaller
