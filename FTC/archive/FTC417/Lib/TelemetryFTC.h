...
//
// This file contains support for sending telemetry records in real time
// to a Microsoft Excel spreadsheet. Actually recording the telemetry
// requires the use of the TelemetryFTC program running on the PC; see
// www.ftc417.org/TelemetryFTC for details on how to get that.
//
// Basic Telemetry:
// ---------------
//
...
//
// etc. calls in order to accumulate data of various types in your
// record, then call
//
//      TelemetrySend();
//
// to actually transmit the record. Data rates of up to around a
// thousand telemetry records per second should be possible.
...
//      TelemetryInitialize();
//
// once, before calling any other telemetry function. And when you're
// done, you can call optionally call TelemetryDone(); telemetry records
// transmitted after a TelemetryDone() (maybe in the next run of your
// program) will appear in a separate, new Excel spreadsheet workbook.
//
...
//
// and one must be sure during telemetry transmission to *not* at that
// momsent be debugging over Bluetooth using RobotC. Additionally, if
// USB transmission is not needed, some small performance improvement
...
//
// When sending data over Bluetooth, the NXT just spontaneously and
// immediately sends out the data over the radio; if no TelemetryFTC
// program is connected and listening, the data just vanishes into the
// night. However, for obscure technical reasons, when the USB
// approaches are used, TelemetryFTC on the PC must poll the NXT
// periodically to see if new records are available. By default,
// TelemetryFTC only polls every so often (currently 30 ms). If, when
// using USB, you find you want telemetry records to be polled faster
// than that, call
//
//      TelemetrySetUSBPollInterval(ms);
//
// where ms is how often the poll should occur (in milliseconds), and
// where a value of zero indicates that polling should be carried out as
// fast as possible. If you're actually running up against this issue,
// then simply calling
//
...
//
// Accumulation and transmission of telemetry records can be temporarily
// suppressed by calling
//
//      TelemetryEnable(false);
//
// and can be unsupressed by later calling TelemetryEnable(true). With
// this technique, you can leave TelemetryAddInt32(),
// TelemetryAddString(), etc and TelemetrySend() calls in your code on a
// permanent basis, and only selectively later decide when you're
// actually interested in really transmitting. Telemetry transmission is
// enabled by default.
//
// By default, telemetry is recorded in the first sheet of the Excel
// spreadsheet workbook. Optionally, you can direct a telemetry record
// to a different sheet in the workbook simply by calling
//
...
//      TelemetrySend();
//
// 'sheetNumber' is the zero-based index of the sheet to be
// used. TelemetrySend() is thus equivalent to TelemetrySendSheet(0).
//
// Finally, it's very often the case that, in order to reliably
// determine whether all telemetry records have been recorded in Excel
// and that none have been lost, and to permanently record the order in
// which the records were created, it's worthwhile to tag each telemetry
// record with its own unique, monotonically increasing serial number,
// usually as the first datum in the record. This paradigm is so common,
// in fact, that we've defined for you the variable
//
//      telemetry.serialNumber
//
// so you have a place to do this. For example, if you want to send two
// integer pieces of data in each telemetry record, then sending the
// serial number along with them, together with a time stamp (also often
// handy), would look something like this:
//
//      // at startup do:
...
//      telemetry.serialNumber++;
//
// Note that telemetry records are of limited capacity. If you try to
// add a datum to the accumulating record and it won't fit, the attempt
// at adding will simply be ignored, as will all further data until the
// record is ultimately sent. Thus, your record when transmitted will
// have fewer columns than expected.
//
// Definitions:
// -----------
//
// Below are the defintions of the functions, etc. that you may use in
// your program.  DO NOT USE any definitions that begin with an
// underscore ('_'), as these are internal to the implementation of how
// telemetry works, and may change in a subsequent release of this
...
// are a means by which particular parts of the RAM memory of the NXT
// are exposed to RobotC programs for reading and writing (using the
// nxtReadIOMap and nxtWriteIOMap APIs). By this means, access to
// various particular low-level parts of the firmware state is possible.
...
typedef struct {
    ...
    byte InPtr;
    // in UsbOutBuf, this is #bytes in Buf that should be
    // xmitd. c_comm.c(278) (at least in the *lego* firmware; robotc is
    // seemingly different)
    byte OutPtr;
    byte Spare1;
    byte Spare2;
...
_TelemetrySend_(_TELEMETRY &t, int isheet, bool fFinalize = true) {
    ...
    if (t._isActive) {
        ...
        if (t._useBluetooth) {
            // We are to spontaneously transmit the record as soon as
            // Bluetooth is free
            while (bBTBusy) {
                EndTimeSlice();
            }
        ...
        if (t._useUSB) {
            // Append the current contents of the telemetry buffer to
            // IoMapComm.HsInBuf. This will be readable remotely using a
            // Poll Command (0xA2) with buffer number == 0x00 (Poll
            // buffer) [sic].
            short ioResult = 0;
            ...
            if (cbToWrite == cbWriteWanted) {
                // Due to circular wrapping, we might have to write the
                // data in two chunks. How much will we write in each?
                const int cbChunkFirst = _Min(cbToWrite, cbBuffer - ibInPtr);
                const int cbChunkSecond = cbToWrite - cbChunkFirst;

                // Write each chunk. Note that we can only write in
                // quanta of 64 bytes at a time, as that is all that the
        ...
        // Reset things so that the next telemetry record will be
        // assembled correctly
        _TelemetryResetRecord_(t);
    }
}
