...
//
// Kudos and gracious thanks to Xander Soldaat (mightor_at_gmail.com)
// for his original 3rd party driver framework, without the guidance and
// instruction of which the code herein would not have been possible.
...
// type 'SENSOR' instead of ACCELSENSOR, ANGLESENSOR, etc. So we do a
// C++-like base-class cast on the parameter.
...
//-----------------------------------------------------------------------------
typedef struct {
    COMMON_SENSOR_DATA float deg;       // the actual, gearing-compensated value of the sensor (NB: NOT normalized to +-180 deg)
...
//-----------------------------------------------------------------------------
typedef struct {
    COMMON_SENSOR_DATA int red, green, blue;    // current values
...
typedef struct {
    COMMON_SENSOR_DATA BOOL fReadProcessed;     // if true, read processed values; false raw values
...
//-----------------------------------------------------------------------------
typedef struct {
    COMMON_SENSOR_DATA float bias;      // the offset bias to apply to raw readings from the sensor. Will be ~620ish, typically
...
typedef struct {
    COMMON_SENSOR_DATA int value;       // current reading if the mag, compensated by the bias
    int valuePrev;
...
//-----------------------------------------------------------------------------
typedef struct {
    COMMON_SENSOR_DATA int cm;  // current reading in cm
...
//-----------------------------------------------------------------------------
typedef struct {
    COMMON_SENSOR_DATA BOOL fValue;
} TOUCHSENSOR;
...
{                                                                       \
    ...
    if (!FMux(sensor)) {                                                \
        if ((SensorType[SID_DIRECT(sensor)] != sensorTouch) ||          \
            SensorMode[SID_DIRECT(sensor)] != modeBoolean) {            \
