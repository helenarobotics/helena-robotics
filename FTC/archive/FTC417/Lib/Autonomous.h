...
// time optimization (when we're not actually carrying batons, of
// course).
BOOL fScorePreloadedBatons = false;

// Do we (true) push the rolling goal to mid-field to try to block
// others or (false) just push it around enough to be able to score
// reliably and to get it out of our own way.
...
// If fGoForMiddleDispenser is false, do we (true) go for the gusto of
// trying in the 'classic' manner to dispense from the left dispenser or
// do we just instead go balance on the bridge (false)
...
DoAutonomous() {
    ...
    // Validate parameters: we can't do the classic strategy from the
    // right side
    if (STARTING_SIDE_RIGHT == startingSide) {
        fGoForMiddleDispenser = true;
    }
    // Set a reflection scale so we can have this code work from both
    // left and right
    const int sideReflection = (STARTING_SIDE_LEFT == startingSide ? 1 : -1);

    // Turn a little bit right so that we'll edge away from the wall,
    // which we need to do before we can complete a 90 deg turn.
    ...
    // Turn the remaining amount necessary to be square on at the
    // rolling goal
    ...
    // Drive to the goal and nudge up against it. Exactly how far we
    // push it depends on what our intent here is.
    float cmExtra = 0.0;
    if (fPushGoalToMidField)
        cmExtra = 46.0;
    else if (fGoForMiddleDispenser)
        // we need to get the goal out of the way of the bridge
        // (distance to be tuned)
        cmExtra = 25.0;
    else
        // still worth it to deal with the castors? (distance to be tuned)
        cmExtra = 5.0;
    ...
    wait1Msec(100);
    RET_IF_FAIL(DriveForwards(cmToGoalSlow, DRIVE_POWER_SLOW + 10),
                "slow to goal");
    ...
    if (fGoForMiddleDispenser) {
        ...
        // Use the front sonic to get a good read on how far we are from
        // the wall, then drive until to our desired distance.
        int cmFromWallCur = ReadSonic_Main(sensSonicFront, true);
        int cmFromWallDesired = 37;
        RET_IF_FAIL(DriveForwards(cmFromWallCur - cmFromWallDesired),
                    "to desired distance");

        // Turn parallel to the wall, pointing to the left side of the
        // field
        ...
        RET_IF_FAIL(DriveForwardsAndStraightenUsingRightSonic(
                        -(72 + 33) * sideReflection + cmExtra,
                        DRIVE_POWER_FAST), "straighten");
        ...
        if (PositionUsingEOPD(DISP_MED, cmFromWallCur, RIGHT_SIDE)) {
            // Dispense things
            if (DispenseFromEOPDPosition(DISP_MED, cmFromWallCur,
                                         SPILL_BATONS)) {
        ...
        // Drive until the sonic is in range, then read the sonic to see
        ...
        // Get up on the cliff before starting to look for white, as the
        // leading edge of the cliff sometimes reads as white.
        RET_IF_FAIL(DriveForwards(20, DRIVE_POWER_FAST), "OnToCliff");

        // Now we should be aligned to drive straight up the ramp.
        // Drive up the ramp until we hit the white line at the lip of
        // the cliff.
        RET_IF_FAIL(DriveToWhiteLine(80, 0.0, BACKGROUND_BLACK),
                    "DriveToWhiteLine");

        // Check the sonic sensor to see if there's someone in our way
        // or not.  Note that we want the space in front of the
        // dispenser to be clear, but we are ok with an opposing robot
        // still sitting there in its initial eighteen inch
        // square. Thus, the distance checked here is subtle.
        int cmFront = ReadSonic_Main(sensSonicFront, false);
        TRACE(("bot in way? %dcm", cmFront));

        // REVIEW: threshold value needs verification
        BOOL fRobotInTheWay = (cmFront < 90) || !fGoForLeftDispenser;
        if (!fRobotInTheWay) {
            // To which dispenser are we headed?
            DISPENSER disp =
                (TEAM_COLOR_RED == teamColor) ? DISP_LOW : DISP_HIGH;
            ...
            BACKGROUND background =
                (TEAM_COLOR_BLUE ==
                 teamColor) ? BACKGROUND_RED : BACKGROUND_BLUE;
            GoBalanceOnBridge(0.0, background);
            //
            // Be silly. Note that if we are the blue team, then we
            // balance on the red bridge.
            //
            TraceElapsed();
            if (TEAM_COLOR_BLUE == teamColor)
                PlayMusic(musicLadyInRed, 120, NOTE_QUARTER);
        }
    }
...
// From the starting position, navigate to the rolling goal and dump our
// preloads.  Return through the angleCur OUT parameter the angle we
// believe we have turned through in the process.
BOOL
DoAutonomousPreloads(OUT ANGLE &angleCur) {
    ...
    // Give feedback as to which team color we are. This message is here
    // (and not earlier) for historical reasons: at one time, we
    // actually had to drive across the line around the starting corner
    // detecting it with a color sensor in order to learn which team
    // color we were.
    ...
    // Extend the preload arm so we can turn and see if touch sensor
    // triggers against the rolling goal.
    ExtendPreloadArm();
    ...
    RET_IF_FAIL(TurnRight(angleSweepMax, TURN_POWER_MEDIUM, stop),
                "TurnRight");
    ANGLE angleSweepActual =
        (stop.fStopConditionReached ? (1.0 -
                                       stop.fractionRemaining) : 1.0) * angleSweepMax;

    // Back up a bit to get the preload arm right over the goal
    const ANGLE angleSweepAdjust = -2.0;
    InitializeStopConditions(stop);
    RET_IF_FAIL(TurnRight(angleSweepAdjust, TURN_POWER_FAST, stop),
                "TurnLeft");
    ...
    // Point towards the wall in order to get a good reading from the
    // front sonic We want to effect a 90 degree turn from our initial
    // starting position, so we turn through whatever angle remains of
    // that from our current angle
    ANGLE angleFudge = 0.0;
    angleCur =
        angleInitialTurn + angleSweepActual + angleSweepAdjust + angleFudge;
...
DoAutonomous() {
    ...
    // Record the initial starting position in the trancript. This is
    // helpful for debugging near misses on the preloads
    if (true) {
...
#    endif
    ...
    // Drive forwards until we get close to the wall. We go a certain
    // distance based on the ultrasound (it's as close as we can
    // reasonably read?), and then a little bit more.
    ...
    RET_IF_FAIL(DriveForwardsAndStraightenUsingLeftSonic(55, DRIVE_POWER_FAST),
                "DriveForwardsAndStraightenLeft");

    // Get up on the ramp before starting to look for white, as the
    // leading edge of the ramp sometimes reads as white.
    RET_IF_FAIL(DriveForwards(20, DRIVE_POWER_FAST), "OnToRamp");

    // Now we should be aligned to drive straight up the ramp.  Drive up
    // the ramp until we hit the white line at the lip of the cliff.
    RET_IF_FAIL(DriveToWhiteLine(70, 0.0, BACKGROUND_BLACK),
                "DriveToWhiteLine");

    // Check the sonic sensor to see if there's someone in our way or
    // not.  Note that we want the space in front of the dispenser to be
    // clear, but we are ok with an opposing robot still sitting there
    // in its initial eighteen inch square. Thus, the distance checked
    // here is subtle.
...
#    endif
    ...
    if (!fRobotInTheWay) {
        ...
        // Drive on to mats at the other side of the low goal. Make sure
        // that we clear the left sonic past the dispenser so that at
        // the low dispenser we still read off the wall.
        RET_IF_FAIL(DriveForwardsAndStraightenUsingLeftSonic(
                        72, DRIVE_POWER_FAST), "AfterCliff");
...
#    endif
        if (cm < CmDispenseCanTurnMin(disp)) {
            //distance from center of rotation to the wall
            float dcmCenterRotation = cm + 16;
            //half diagonal of the bot, for calculation reasons
            float dcmHalfDiagonal = 30;
            //half length over hypotenuse
            ANGLE radAngleBeta = asin(21. / dcmHalfDiagonal);
            ANGLE radAngleTheta =
                asin(dcmCenterRotation / dcmHalfDiagonal) - radAngleBeta;
            RET_IF_FAIL(AdjustLaterallyUsingAngle((float)(CmDispenseCorrect
                                                          (disp) - cm), radiansToDegrees(radAngleTheta)),
                        "AdjustLaterally");
        } else if (cm < CmDispenseMin(disp)) {
            TRACE(("too close: cm=%d", cm));
            RET_IF_FAIL(AdjustLaterally((float)(CmDispenseCorrect(disp) -
                                                cm)), "AdjustLaterally");
        } else if (cm > 70) {
...
#    endif
        ...
        if (cm >= CmDispenseMin(disp) && cm < CmDispenseMax(disp)) {
            ...
            // rest a bit (may not be needed: we're trying to see why
            // the calibrated values sometimes fail)
            wait1Msec(200);
        ...
        BACKGROUND background =
            (TEAM_COLOR_BLUE == teamColor) ? BACKGROUND_RED : BACKGROUND_BLUE;
        GoBalanceOnBridge(0.0, background);
        //
        // Be silly. Note that if we are the blue team, then we balance
        // on the red bridge.
        //
        TraceElapsed();
        if (TEAM_COLOR_BLUE == teamColor)
