...
// ServoArm.h
//
// Functionality related to moving the arms on the bot. Note that this
...
// Enums naming the various servos, logical arm positions, and
// dispensers involved in dispensing. These are use to parameterize
// distance and svpos queries.
typedef enum {
    SVOJNT_ELBOW = 0,
    SVOJNT_SHOULDER,
    SVOJNT_ORDER,
...
#    if HAS_ARM_SERVOS
    if (CmDispenseMin(disp) <= cm && cm < CmDispenseMax(disp)) {
        ...
        if (DISP_HIGH == disp) {
            ...
            svposShoulder =
                rgsvposHighDispenser[dcm][isvpos + SVOJNT_SHOULDER];
            fSuccess = true;
...
int
LookupServoPosition(DISPENSER disp, int cm, ARMPOS armPos, SVOJNT svojnt) {
    // Return the svpos for the indicated arm position of the indicated
    // servo at the indicated dispenser, given that the left ultrasound
    // reads cm. If the parameters are out of bounds, return a negative
    // number.
    //
    ...
    if (LookupServoPositions(disp, cm, armPos, OUT svposRotation,
        ...
        case SVOJNT_SHOULDER:
            return svposShoulder;
        }
    }
    return -1;
}
...
// the EOPD on the end of the arm to sense the location of the baton
// dispenser.
BOOL
...
#    elif ROBOT_NAME==ROBOT_NAME_FTC417_2010_V11
    ...
    // Raise the top of the arm to where it needs to be to clear the
    // wall as we rotate
    ...
    // From the Reference point, we have to subtact 10cm from the
    // distance from the wall to get to our point. We add 5 because the
    // reference point is NOT the origin from the geometry and the y
    // coordinate stays the same. This gets the arm right in front of
    // the dispensor, not in dispensing mode.
    //
    // Move the arm down to the indicated point, which is only
    // approximately horizontal in the forewarm. (Note: MoveArmTo does
    // the WaitForServos internally.)
    MoveArmTipTo(cm - 10 + 5, 16.5, MOVEARMTIP_DIRECT);

    // Level out the forearm so the EOPD sensor on the end is
...
PackArmSafelyFromAnywhere(BOOL fConservativelyRotate) {
    ...
    // basically linear path from the current location into the packed
    // one intersects the side of the robot
    //
    // Needs more thinking: nice idea, but very unexpected if manual
    // packing moves have been made in interim.
...
{                                                                       \
    ...
    ARMPOINT pt;                                                        \
                                                                        \
    /*                                                                  \
     * The first point in the path is the EOPD position, which is where \
     * we currently are.                                                \
     */                                                                 \
...
// We're at the EOPD position. Move to dispensing, reporting the path we
...
#    if HAS_ARM_SERVOS
    ...
    if (fSuccess) {
        ...
        // that's the EOPD position, which we're already at
...
#    if HAS_ARM_SERVOS
    ...
    if (fRotSvoControllerJustPowered) {
        // If the servo controllers just powered on, if we try to move
        // ANY servo on a controller, then ALL the servos on that
        // controller will move, whether or not we've told the
        // controller that we'd like them to go to something other than
        // their default '128' svpos. That's to be avoided, as '128' is
        // almost never exactly what we want it to move to (though it's
        // indeed safe (assuming range of motion ok), since paired
        // servos won't fight (much) and rotational servos are stopped.
        //
        // So what we do is 'move' ALL our servos to the packed
        // location, but don't 'WaitForServos' until they're all
        // done. We will, though, assume that the arm was put in what's
        // close to the packed position.
        //
        PackArmSimply();
