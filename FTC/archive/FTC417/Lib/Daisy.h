...
// Support for manipulating the daisy chain of HiTechnic motor and servo
// controllers.
...
// The I2C device address of a controller depends on the placement of
// the controller in the daisy chain. The first device closest to the
// NXT will have an address of 0x02, next one 0x04, then 0x06, and
// finally 0x08 for the fourth and final device.
...
{                                                                       \
    ...
    if (jController <= 0)                                               \
        fSuccess = false;                                               \
                                                                        \
    /* Make sure we have the right sensor type that we want, but try */ \
    /* to avoid (paranoia) setting the sensor type unnecessarily for */ \
    /* fear of inadvertent side effects */                              \
...
typedef enumWord {
    // Low order two bits are a 2-bit field that controls how the motor
    // is managed
    ...
    MOTORMODE_FORCE = 0,    // ie: nothing: we rely on the fact that
                            // we're checking for stalls often enough
                            // that this isn't an issue
} MOTORMODE;

// Each motor controller keeps some state about each of its (possibly
// two) attached motors
typedef struct {
...
// Declaration of the motor controller variables. Up to four motor
// controllers and servo controllers may be attached in a daisy chain,
// in any combination, so a chain can have at most four controllers,
// which define here. NOTE: it is perhaps the case (though unverified)
// that one can have daisy chains on more than one of the sensor ports;
// that scenario (if more than four motor contollers were used) would
// require that this array be increased in size.
...
// Update the target encoder settings, the motor modes, and the motor
// power levels in this controller
void
...
ReadLegoEncoder(MOTORCONTROLLER &controller, int imtr) {
    /*
     * We mult lego encoders values by 4 so all encoders (Tetrix &Lego)
     * are in 1440 ticks per rev
     */
    if (controller.rgmtr[imtr].fActive) {
        /*
         * Read the lego motor encoder. Its value may have wrapped, and
         * what we do to deal with that (ie: to unwrap the value)
         * depends on whether we were going forward (and thus expecting
         * the encoder value to increase) or backward, (expecting it to
         * decrease).
         */
        ...
        /*
         * Update the accumulated unwrapped encoder value. A persnicity
         * issue is that the encoders sometimes continue to move even
         * after power has been shut off, but the SetMotorPower function
         * takes care to remember which direction the motors were going,
         * even if they're off now.
         */
...
// Read the current values of this controller's motor's
// encoders. Because it's cheap and convenient, at this moment we also
...
// Communicate with this motor controller in such a way that it doesn't
// time out
...
// From what we understand, the motor controller only updates its state
// on a tick of an internal clock which ticks every n ms (the value of n
// is not clear). Thus, if you send a command (like a mode change) to
// the controller, you need to guarantee that a tick occurs in order to
// ensure it's effect before doing an action (like another mode change)
...
//
// The Step time field sets the step time for the servo channel which
// has the furthest to move. Other servo channels which are not at their
// designated positions yet will run at a slower rate to ensure they
// reach their destination positions at the same time. This value may
// range from 1 - 15. If the field is set to 0, no timing s applied (ie;
// the servos move at their maximum rate) and the
// QueryServoMovementComplete() request no longer returns sensical
// information.
//
// That all said, the servos themselves don't electrically actually have
// any means by which they can report their position. So, the servo
// controller is just guessing as to when the servos will be done. We
// can guess just as well, even better, what with our knowledge of
// apogee, calibration step, and so on. So we don't use the servo
// controller waits by default.
//
...
// If the controller doesn't yet know about any of the requested servo
// positions, then tell it.
void
...
// Touch each of the servo controllers so they don't time out and power
// themselves off. Call this every so often (quite often is fine).
#define TickleServos(msNow)                                             \
{                                                                       \
    /* HiTechnic servo controllers have 10 sec time out. */             \
    /* So we use a threshold close to that */                           \
    /* But not too close. */                                            \
    ...
    if (0 == msTickleServosPrev || msNow - msTickleServosPrev > 1000) { \
        ...
        for (int isvoctlr = 0; isvoctlr < isvoctlrMax; isvoctlr++) {    \
            if (rgsvoctlr[isvoctlr].fActive) {                          \
                if (msNow - rgsvoctlr[isvoctlr].msLastSend >            \
                    msThreshold) {                                      \
                    BOOL fSuccess = true;                               \
                    BOOL fComplete;                                     \
    ...
    if (fSuccess) {
        ...
        fSuccess =
            i2cSendReceiveDaisy(controller.link, reqDaisy, repDaisy,
                                isvcMax + 1);
        if (fSuccess) {
            // Upon a power cycle (of the 12v power?) servo positions
            // are all reported as 128 and the controller mode is
            // disabled. Detect that condition here.
            ...
            for (int isvc = 0; isvc < isvcMax; isvc++) {
                // 128 is the default used by the servo
                // controller. Makes sense, esp. ack'ing that servos are
                // sometimes paired.
                controller.rgsvc[isvc].svpos = 128;
            }
            controller.pwmMode = SVOPWM_DISABLE;
            controller.fJustPowered = true;
        }
