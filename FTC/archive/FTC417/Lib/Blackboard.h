...
// how many times has the blackboard been updated?
int32 cBlackboardUpdate = 0;
// time at which the last blackboard update occurred
int32 msBlackboardUpdate;
...
{                                                                       \
    /* The rate at which ultrasonic sensor can be read is limited */    \
    /* both by the speed of sound and internal implementation issues: */\
    /*  http://www.robotc.net/forums/viewtopic.php?f=1&t=2224&hilit=ultrasonic      */ \
    /* In short, the ultrasonic sensors take one reading in ~77ms, */   \
    /* so if we want a new reading we need to be at least that long */  \
    /* since the last one; we use 80 to be sure. More correctly, */     \
    /* that was the native RobotC read speed; with us here using  */    \
    /* own I2C routines, we might be able to do faster */               \
    /* (see end of the thread above).  */                               \
...
// We read until we get two readings in a row with basically the same
// value.  Note that this function can a very long time to complete.
// Return cmSonicNil if there's no reading available; that will never
// happen if fAllowInfinite is false (we'll spin until that doesn't
// happen).
int
CALLEDBY(iTaskMain)
ReadSonic_Main(SONICSENSOR &sensor, BOOL fRequireFinite) {
    ...
    int cmCur = sensor.cm;
    int cmPrev;
    for (int iRetry = 0; iRetry < 10 || (fRequireFinite &&
                                         (cmSonicRawNil == cmCur ||
        ...
        cmCur = sensor.cm;
        if (fRequireFinite &&
            (cmSonicRawNil == cmCur || cmSonicRawNil == cmPrev))
            continue;
...
// Do the work of updating the blackboard. Note that (per Dave
// Schilling) we can't actually read the compass (or any of the other
// sensors?) faster than 100Hz, so we enforce a throttle here.  That
// won't be significant when we're called from the BlackboardTask (it
// doesn't call that often), but might be when we're called manually via
// SpeculativelyUpdateBlackboard().
//
// REVIEW: further research indicates that's probably hookum, esp. when
// sensor are on a mux.  But actual removal of that throttle needs more
// testing before it can be implemented.
void CALLED_WITH_BB_LOCK
UpdateBlackboard() {
    const MILLI msBlackboardThrottle = 8;       // was 11
    MILLI msNow = nSysTime;     // read the current clock

    // We do the gyro's every time through because they're really
    // sensitive time-wise.
...
// We'd like to just 'suspendTask' & 'resumeTask', but apparently those
// functions aren't functional in RobotC
