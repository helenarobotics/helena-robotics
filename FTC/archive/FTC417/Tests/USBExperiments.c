...
void
Bar(BDDEVICETABLE &data) {
}
void
Bar(BDCONNECTTABLE &data) {
}
void
Bar(BRICKDATA &data) {
}
...
ComputeFieldOffsets() {
    ibBtDeviceTable = 8;
    ibBtConnectTable =
        (ibBtDeviceTable + (sizeof(BDDEVICETABLE)) * SIZE_OF_BT_DEVICE_TABLE);
    ibBrickData =
        (ibBtConnectTable +
        (sizeof(BDCONNECTTABLE)) * SIZE_OF_BT_CONNECT_TABLE);
...
void
DumpCommMod(int ibFirst = 0, int cbMax = cbIOMAPCOMM) {
    short ioResult = 0;
    ...
    for (int ib = ibFirst; ib < ibMax; ib += sizeof(rgb)) {
        ...
        for (int i = 0; i < cb; i += 8) {
            ...
            StringFormat(sCharsLow, "%c%c%c%c", Cleanse(b0, ibMax, ib + i + 0),
                Cleanse(b1, ibMax, ib + i + 1), Cleanse(b2, ibMax, ib + i + 2),
                Cleanse(b3, ibMax, ib + i + 3));
            StringFormat(sCharsHigh, "%c%c%c%c", Cleanse(b4, ibMax,
                    ib + i + 4), Cleanse(b5, ibMax, ib + i + 5), Cleanse(b6,
                    ibMax, ib + i + 6), Cleanse(b7, ibMax, ib + i + 7));

            TRACEWAIT(("%d %d: %s|%s %s%s", cb, ib + i, sBytesLow, sBytesHigh,
                    sCharsLow, sCharsHigh));
        }
    }
}
...
void
PostMessage(string str) {
    _TelemetryAddString_(telemetry, str);
    short ioResult = 0;
    if (true) {
        ioResult =
            cCmdMessageAddToQueue(telemetry.mailbox, telemetry.rgbMsg[0],
            telemetry.cbMsg);
    } else {
        while (bBTBusy) {
            EndTimeSlice();
        ...
        ioResult =
            cCmdMessageWriteToBluetooth(telemetry.rgbMsg[0], telemetry.cbMsg,
            telemetry.mailbox);
    }
    TRACE(("ioResult=0x%04x mb=%d cb=%d", ioResult, telemetry.mailbox,
            telemetry.cbMsg));
...
CircularWriteTelemetryToHsInBuf() {
    ...
    ubyte rgbPtr[2];
    nxtReadIOMap(strCommMap, ioResult, rgbPtr[0], dibHsInBuf + dibHSBUFInPtr,
        2);
    ...
    if (cbToWrite == telemetry.cbMsg) {
        ...
        _CircularWriteChunk_(telemetry.rgbMsg, 0, dibHsInBuf + ibInPtr,
            cbChunkFirst, ioResult);
        _CircularWriteChunk_(telemetry.rgbMsg, cbChunkFirst, dibHsInBuf,
        ...
        nxtWriteIOMap(strCommMap, ioResult, rgbPtr[0],
            dibHsInBuf + dibHSBUFInPtr, 1);
