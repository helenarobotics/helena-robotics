...
void move(int iSpeed, int nTicks)
{
	...
	while((abs(nMotorEncoder[mLeft]) < vEncoderTic || abs(nMotorEncoder[mRight]) < vEncoderTic) && !end)
	{
		...
		{
			if(/*nPgmTime - vStartTime > 500 && */abs(vCurrLeftPos - vPrevLeftPos) < gcMotorStop)
				end = true;

			if(/*nPgmTime - vStartTime > 500 && */abs(vCurrRightPos - vPrevRightPos) < gcMotorStop)
				end = true;
...
task main()
{
  ...
  waitForStart();
	...
	servo[sIr] = 255;
  movein(40, 15);
  turn(65, 32, L_CODE);//Initial move;
