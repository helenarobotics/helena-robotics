...
public class Region {
    ...
    public enum HoopLocation { unknown, left, top, right };       // top is also middle hoop (located slighly higher than left or right)
...
private int partition(dataPoint arr[], int left, int right){
	...
	if (right <= left)        //  watch out for  case of zero-length vector.  If this happens, partition blows up
	    return;
