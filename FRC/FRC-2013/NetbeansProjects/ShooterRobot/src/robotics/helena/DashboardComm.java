...
//    public static double rightMotorPower;
    ...
    public void send() {
        ...
        //Joystick 1
        s.putNumber("Joystick 1 Direction", joy1Direction);
        s.putNumber("Joystick 1 Magnitude", joy1Magnitude);
        s.putBoolean("Transmission", shifterStatus);

        s.putInt("Mode", mode);

        //Joystick 2
        s.putNumber("Joystick 2 X", joy2X);
        s.putNumber("Joystick 2 Y", joy2Y);
        s.putNumber("Joystick 2 Throttle", shooterThrottle);
        s.putBoolean("BallFeeder", shooting);

        //For Field - Gyro
//        s.putNumber("Gyro", gyroAngle);

        //Accelerometer
//        s.putNumber("Accel X", accel.getAcceleration().kX);
//        s.putNumber("Accel Y", accel.getAcceleration().kY);
//        s.putNumber("Accel Z", accel.getAcceleration().kZ);

        //RPM Sensor
        s.putNumber("RPM 1", (int)rpmBottom);
        s.putNumber("RPM 2", (int)rpmTop);
        s.putNumber("RPM 1 Target", (int)rpmTopTarget);
