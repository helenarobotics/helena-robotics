...
task main()
    ...
    PlayHappy();
    //
    fDisplaySonic = true;               // cause DisplayTask to show sonic readings
    //
    ...
    for (;;)
        ...
        // A never-taken conditional so that ALL the cases below are uniformly 'else if'
        if (0)
            {
            }
        ...
        //----------------------------------------------------------
            ...
            {
            // Go! Zero the encoders so that they're interesting at the end of the drive
            ZeroEncoders();
