...
#define degShoulderCalibrate     90     // degrees where zero is
                                        // shoulder flat, 90 is shoulder
                                        // vertical, >90 is shoulder
                                        // snugged into packed
...
// they're 3" wheels, even though they are treads
#define CM_WHEEL_CIRCUMFERENCE          (3.0 * 2.54 * PI)

// the mechanical advantage from motor to wheel
#define MOTOR_GEARING                   1.5
...
InitializeMotors(IN OUT STICKYFAILURE &fOverallSuccess) {
    ...
    //      two on each channel of motorControllerAux, each two wired in
    //        parallel on their respective channels
    //
    // These appear to us as four *logical* motors: we can't 'see' that
    // there are two motors on each channel of motorControllerAux. Given
    // how the chain drive works, we need the motors on
    // motorControllerAux to run in the reverse direction from the
    // corresponding motors on motorController.
    //
...
// One MUST #define macros like this for each of the sensor one has as
// elsewhere in the code conditional compilation tests for their
// presence or absence. Morover, one MUST use the stock set of macro
// names. See main.h for further details.
...
InitializeSensors(IN OUT STICKYFAILURE &fOverallSuccess) {
    ...
    // teeth. Thus, the angle sensor rotates TOO FAST. Set to a value
    // less than one to compensate.
    //
    // Further, we wish to reverse the direction of the sensor, so that
    // positive values correspond with increasing duration in which the
    // rotor has been operating in its normal unload dispenser / score
    // batons rotation. So we negate the gearing constant.
    ...
    // Our gyro sensor has been observed to overread a bit: e.g.: what
    // the gyro thinks is 360.0 degrees is in fact a bit less. So we
    // need to scale back the readings.
...
// On v11, we have an 'intermediate intermediate' position, but not so
// much on v12
...
// Speed constants for the rotor. Note that we've got a whole bunch of
// headroom numerically, but in practice, we've found that the speed
// response curve is highly non-linear, and peak speed is reached long
// before the numerical range is exhausted.
...
//
// When dispensing, how many degrees from initial baton engagement until
// it's at the mag sensor detect position?
...
// A bevy of constants about the length and angles of various parts of
...
// Convert angles to svpos in this state, and back again. In the v12
// model, larger angles are higher (unlike v11). This corresponds to
// numerically higher svpos as well (to be confirmed).
...
// Table of motor encoder deltas necessary to achieve certain angles of
// turn.  This table was created by empirical observation of the
// robot. Each entry in the table has three entries: a target turn
// angle, then left and right encoder deltas needed to achieve that
// angle. The table must be sorted in increasing order by angle.
