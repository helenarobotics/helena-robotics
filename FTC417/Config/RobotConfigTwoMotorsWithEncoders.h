...
InitializeMotors(IN OUT STICKYFAILURE &fOverallSuccess) {
    // The one motor controller on this robot is controller #1 in the
    // daisy chain of controllers on sensor port S1 (and on this bot,
    // that's the ONLY controller in said daisy chain).
    ...
    InitializeMotor(fOverallSuccess, motorLeft, motorController, 1, "left",
        MOTORPROP_NONE | MOTORPROP_ENCODER);
    InitializeMotor(fOverallSuccess, motorRight, motorController, 2, "right",
        MOTORPROP_REFLECTED | MOTORPROP_ENCODER);
}
