...
const ENCOD dencDriveTolerance = Rounded((float)CM_DRIVE_TOLERANCE / (float)cmPerEncoderTick, ENCOD);
...
void CALLEDBY(iTaskMain) RampDrivePower(DRIVESTATE& state, float fraction)
    {
...
void CALLEDBY(iTaskMain) RampDrivePower(DRIVESTATE& state)
    {
    if (state.absdenc > DRIVE_RAMP_THRESHOLD || !USE_DRIVE_RAMP)
        RampDrivePower(state, 1.0);
    else
        {
        float encoderFraction = (float)state.absdenc / (float)DRIVE_RAMP_THRESHOLD;
...
BOOL CALLEDBY(iTaskMain) DriveForwards(float cmDistance, STOPCONDITIONS& stop, int16 power)
...
FunctionStart:
    ...
    if (Abs(cmDistance) >= CM_DRIVE_TOLERANCE)
        ...
        state.fractionRampTo = (float)state.powerRampTo / (float)state.powerDrive;
        //
        TRACE(("------------------------"));
        ...
        for (;;)
            ...
            if (Sign(state.denc) != Sign(dencPrev))
                {
                TRACE(("drive: overshot: cur=%d prev=%d", state.denc, dencPrev));
                break;
                }
            //
            RampDrivePower(state);
...
BOOL DriveForwards(float cmDistance)
    {
    STOPCONDITIONS stop;
    InitializeStopConditions(stop);
...
BOOL DriveForwardsToSonic(int cmSonic, float cmExtra)
...
BOOL DriveForwards(float cmDistance, int power)
    {
    STOPCONDITIONS stop; InitializeStopConditions(stop);
