...
typedef enumWord
    ...
    SMUXSTAT_HALT       = 0x04,
    SMUXSTAT_ERROR      = 0x08,
    SMUXSTAT_NOTHING    = 0xFF,     /*!< Status hasn't really been set yet */
    } SMUXSTAT;
...
typedef enumWord
    ...
    MUXSENST_ANGLE       = 8,
    MUXSENST_IRSEEK      = 9,
    MUXSENST_NONE        = 0xFF,    // never reported by sensor mux (?); we use internally to denote absense of sensor
    } MUXSENST;
...
// of that command on its state. Wait until the command completes, if necessary.
BOOL SMUXsendCommand(I2CLINK link, SMUXCMD command)
    {
    int msWait = 0;
    BOOL fSuccess = false;
    ...
    if (i2cSendSensor(link, i2cSmuxReq, 0))
        ...
        switch (command)
            {
        case SMUXCMD_HALT:       rgsensormux[link].status = SMUXSTAT_HALT;    msWait = 50;  break;
        case SMUXCMD_AUTODETECT: rgsensormux[link].status = SMUXSTAT_BUSY;    msWait = 500; break;
        case SMUXCMD_RUN:        rgsensormux[link].status = SMUXSTAT_NORMAL;                break;
            }
        fSuccess = true;
        }
    else
...
// Set a mode for a sensor attached to a mux. Note that the mux will be halted
// to do so if necessary, but we'll return it to it's previous state on exit.
void SMUXsetModeChannel(OUT BOOL& fSuccess, I2CLINK link, byte channel, SMUXCHMODE mode)
    {
    ...
    if (fSuccess)
        ...
        if (fSuccess)
            ...
            FormatI2CCmd(i2cSmuxReq, HTSMUX_I2C_ADDR, HTSMUX_CH_OFFSET + HTSMUX_MODE + (HTSMUX_CH_ENTRY_SIZE * channel), mode);
...
BOOL SMUXreadPortChannel(I2CLINK link, byte channel, I2CBUFFER &result, int cb, int dib)
    {
    BOOL fSuccess = false;
    if (FSMUXrun(link))
        {
        FormatI2CReq(i2cSensorReq, HTSMUX_I2C_ADDR, HTSMUX_I2C_BUF + (HTSMUX_BF_ENTRY_SIZE * channel) + dib);
...
void SMUXreadAnalogChannel(I2CLINK link, byte channel, int& result)
    {
    result = -1;
    if (FSMUXrun(link))
        {
        FormatI2CReq(i2cSensorReq, HTSMUX_I2C_ADDR, HTSMUX_ANALOG + (HTSMUX_AN_ENTRY_SIZE * channel));
        if (i2cSendReceiveSensor(link, i2cSensorReq, i2cSensorRep, 2))
            {
            // The analog value is 10 bits. The upper 8 bits are in the first byte and the lower two bits are in the second byte.
...
// Have the sensor mux poke around to find out what's connected to it. Then talk to it to dig out all the types of sensors connected.
...
void InitializeSensorMux(IN OUT STICKYFAILURE& fOverallSuccess, int link)
    {
    if (!rgsensormux[link].fActive) /* avoid multiple initializations */
        {
