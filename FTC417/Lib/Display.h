...
typedef enum { DISPLAY_MODE_STOPPED, DISPLAY_MODE_SIMPLE, DISPLAY_MODE_FULL } DISPLAY_MODE;
...
void CALLEDBY(iTaskMain) DisplayMessage(string szMessage)
    {
...
void CALLEDBY(iTaskMain) DisplayMessageTemporarily(int ms, string szMessage)
    {
    InitializeDisplayIfNeeded();
    ...
    for (;;)
        ...
        if (DISPLAY_MODE_FULL == displayMode)
            ...
            if (fDisplaySonic)
                ...
                RawReadSonic(sensSonicFront); cmFront = sensSonicFront.cm;
                #endif
                #if SensorIsDefined(sensnmSonicBack)
                RawReadSonic(sensSonicBack); cmBack = sensSonicBack.cm;
                #endif
                #if SensorIsDefined(sensnmSonicLeft)
                RawReadSonic(sensSonicLeft); cmLeft = sensSonicLeft.cm;
                #endif
                #if SensorIsDefined(sensnmSonicRight)
                RawReadSonic(sensSonicRight); cmRight = sensSonicRight.cm;
                #endif
                }
        ...
        if ((0 == voltBatteryExternal || voltBatteryExternalAlarmThreshold <= voltBatteryExternal)
        ...
        //
        nxtDisplayTextLine(0, "ext=%1.1f nxt=%1.1f   ", voltBatteryExternal, voltBatteryNxt);
