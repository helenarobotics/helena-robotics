...
// Turn.h
//
// NOTE: The ramping functionality would be critical in a wheel-based
//       bot, but it's causing us stability problems here (esp when the
//       power gets low at the end) so we turn it off for now. Once we
//       get decent PID working, we can probably turn it back on.
...
typedef struct
// Internal state used by the turn logic, one instance for each of the
...
CALLEDBY(iTaskMain)
  RampTurnPower(TURNSTATE &state) {
    ...
    if (!USE_TURN_RAMP || state.fUseGyro ||
        state.absdencCur > state.dencRampThreshold)
...
CALLEDBY(iTaskMain)
  TurnRightCore(ANGLE angle, int16 power, TURNFLAG flags, ENCOD dencLeft,
    ...
    if ((fUseGyro && angle != 0.0) || (dencLeft != 0 || dencRight != 0)) {
        ...
        // Suspend the Blackboard task itself, since our
        // SpeculativelyUpdateBlackboardq call below will do all that
        // stuff on this thread quite nicely and with less contention.
        SuspendBlackboard();

        // Power the motors. Do this *before* messing with the gyro so
        // we don't have that few ms of delay as we start them up: it
        // might be messing with the gyro.
        ...
        if (fUseGyro) {
            ...
            // Make sure we know exactly where the gyro is right
            // now. Remember in a BOOL to later undo the
            // StartingReadingGyroSensor.
            ...
            // Figure out what we want the target gyro angle to be. Note
            // that measured angles are in degrees CCW (i.e.: to the
            // left) so that we need to subtract rather than add 'angle'
            // to compute the target.
            //
            // We also need to compensate for the fact that our gyro is
            // acting up and seems to need an error correction.
        ...
        for (;;) {
            ...
            if (CheckStopConditions(stop)) {
                ...
                if (!fUseGyro) {
                    float leftFraction =
                        (float)stateLeft.dencCur / (float)dencLeft;
                    float rightFraction =
            ...
            if (!fUseGyro) {
                // Figure out if we've hit the target or overshot on
                // each of the motors
            ...
            // Update the blackboard ourselves (recall we suspended the
            // actual blackboard task)
...
void
GetEncoderDeltasForTurnLeft(ANGLE angle, ENCOD &dencLeft, ENCOD &dencRight)
...
// of the indicated magnitude
{
    ...
    // Invariant: the line we're looking for is in the index range
    // [iFirst, iLast].
    int iCur;
    BOOL fFound = false;
    while (true) {
    ...
    // Allow for us to be able to scale the table entries. This has
    // historically been used as a quick hack to adjust mpangledenc
    // table values to adjust for different surfaces, such as carpet vs
    // foam pad.
...
BOOL
TurnRight(ANGLE angleToTurn, int16 powerLevel, float balance) {
    STOPCONDITIONS stop;
