...
void InitializeSensor_(IN OUT STICKYFAILURE& fOverallSuccess, string nm, SENSOR& sensor, int sensnmVal, MUXSENST type)
    ...
    if (FMux(sensor))
        ...
        if (fOverallSuccess)
	        ...
	        if (!fSuccess)
	            {
	            TRACE(("%s: type is:%d want:%d", sensor.displayName, GetSensorTypeMux(SID_MUX(sensor)), type));
	            }
	        }
        }
    else
...
#define StartDetectingSensor_(sensor)                                   \
    {                                                                   \
    if (0 == sensor.cDetectRequest) { sensor.fDetected = false; }       \
...
#else
	void StartDetectingAngleSensor(ANGLESENSOR& sensor) { StartDetectingSensor_(sensor); }
	void StopDetectingAngleSensor(ANGLESENSOR& sensor)  { StopDetectingSensor_(sensor); }
	void StopReadingAngleSensor(ANGLESENSOR& sensor)    { StopReadingSensor_(sensor); }
#endif
...
void ReadEopdSensor(EOPDSENSOR& sensor)
    {
    if (FDoSensorRead(sensor))
        {
        RawReadEopd(sensor);
...
void ReadGyroSensor(GYROSENSOR& sensor, MILLI msNow)
    {
    if (FDoSensorRead(sensor))
        {
...
void ReadMagneticSensor(MAGNETICSENSOR& sensor)
    {
    if (!FDoSensorRead(sensor))
        {
        sensor.value = 0;
        ...
        if (FDoSensorDetection(sensor) && sensor.value >= sensor.threshold)
            ...
            if (VALUECHANGE_DESCENDING==sensor.valueChange && VALUECHANGE_DESCENDING!=sensor.valueChangePrev)
                ...
                if (sensor.isensangleAssociated >= 0)
                    ...
                    ANGLE deg = AngleSensorFromIndex(isensangleAssociated,deg);
...
#else
	void StartDetectingMagneticSensor(MAGNETICSENSOR& sensor)
	    {
	    if (!FDoSensorDetection(sensor)) { ResetMagneticSensorDetection(sensor); }                  \
	    StartDetectingSensor_(sensor);                                                              \
	    }
	void StopDetectingMagneticSensor(MAGNETICSENSOR& sensor) { StopDetectingSensor_(sensor); }
	void StartReadingMagneticSensor(MAGNETICSENSOR& sensor)  { StartReadingSensor_(sensor); }
	void StopReadingMagneticSensor(MAGNETICSENSOR& sensor)   { StopReadingSensor_(sensor); }
#endif
