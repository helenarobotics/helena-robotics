...
void MoveSerovsForScoring_(int svposRot, int svposShoulder, int svposElbow, int svposWrist)
    {
    /* Sanity check */
    ...
    if (svposRot==svposRotCur && svposShoulder==svposShoulderCur && svposElbow==svposElbowCur && svposWrist==svposWristCur)
        return;
...
BOOL FDegUntilNextMagneticBatonSensor(MAGNETICSENSOR& sensor, ANGLE degRotor, OUT ANGLE& degResult)
    {
    BOOL fResult = false;
    degResult = 720.0;      // anything bigger than a full turn will do

    for (int idegDetectionRecord = 0; idegDetectionRecord < cdegMagDetectionRecordMax; idegDetectionRecord++)
        {
        if (sensor.rgfDetectionRecordValid[idegDetectionRecord])
            {
            // How far has the rotor rotated since detection?
            ANGLE degRotated   = degRotor - sensor.rgdegDetectionRecord[idegDetectionRecord];

            // How many degrees remain until we need to attend to it?
            ANGLE degRemaining = DEG_ROTOR_TURN_MAG_DISPENSE_BATON - degRotated;
...
BOOL FDegUntilNextMagneticBaton(ANGLE degRotor, OUT ANGLE& degResult)
    {
    ANGLE degMagRemaining, degMagRemainingAux;
    BOOL fMagRemaining    = FDegUntilNextMagneticBatonSensor(sensMagRotor,    degRotor, degMagRemaining);
    BOOL fMagRemainingAux = FDegUntilNextMagneticBatonSensor(sensMagRotorAux, degRotor, degMagRemainingAux);
...
void ScoreBatonsInRollingGoal(ROLLING_GOAL goal)
    {
    BOOL fCanScore = true;
    //
    ...
    if (!fCanScore)
        ...
        if (!fHaveMagneticBaton)
            ...
            for (;;)
                ...
                ANGLE degRemaining;
                BOOL fAnyMagnetic = FDegUntilNextMagneticBaton(degCur, degRemaining);
                if (!fAnyMagnetic)
                    {
                    ...
                    if (degToCompletion > DEG_ROTOR_TURN_SINGLE_BATON_SCORE*0.5)
                        {
                    ...
                    TurnRotorBy(DEG_ROTOR_TURN_SINGLE_BATON_SCORE, svposRotorSpeed);
                    }
                }
...
BOOL DriveToWhiteLine(float cmDistance, float cmExtra, BACKGROUND background)
    {
...
BOOL GoBalanceOnBridge(ANGLE fieldHeadingZero, BACKGROUND background)
// We're at the white line at the top of the cliff. Go balance on the adjacent bridge!
    ...
    RET_IF_FAIL(TurnRightEncod(TURN_POWER_FAST, -dencBack,dencBack),  "turning left");
...
BOOL PositionUsingEOPD(DISPENSER disp, int cm, SIDE side)
...
// to dispense.
    ...
    RET_IF_FAIL(UnpackDispenserArmForReadingEopd(disp, cm, side), "UnpackEOPD");
...
BOOL DispenseFromEOPDPosition(DISPENSER disp, int cm, BATON_DISPOSITION batonDisposition)
    {
    ...
    switch (disp)
        {
    case DISP_LOW:  yDispenserBottom =  8.3; break;     // measured by tony romano
    case DISP_MED:  yDispenserBottom = 15.5; break;     // measured by tony romano
    case DISP_HIGH: yDispenserBottom = 22.5; break;     // measured by tony romano
        }
