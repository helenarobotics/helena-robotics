...
typedef enum
    { CAPTURE_BATONS, SPILL_BATONS, STOP_BEFORE_DISPENSING } BATON_DISPOSITION;
...
{                                                                       \
    ...
    BOOL fOK = fFirstTime ||                                            \
               GetServoValue(svoRotor) == svposRotorStop ||             \
...
// Given a rotor speed, compute the same speed but in the opposite rotor
// direction
#define ReverseRotorSpeed(speed)        (256 - (speed)) // 128 - ((speed) - 128)

#if USE_ROTOR_STALL_CHECK

// The loop of DoRotorTaskWork runs faster during the act of carrying
// out the flick in order to get more precision on its timing.
...
{                                                                       \
    if (fDoRotorTaskWork &&                                             \
        msNow - msRotorTaskCheckPrev >= msRotorTaskInterval) {          \
        ...
        case ROTOR_STALL_STATE_FLICK_REVERSE:                           \
            /*                                                          \
             * If the reversing has gone on long enough,                \
             * then progress forward again.                             \
             */                                                         \
...
SetRotorSpeed(int svposSpeed) {
    ...
    // If reversing motor, reset stall detection. This avoids a bug in
    // which we accidentally but validly see the same rotor angle
    // position twice (as we retrace our steps) and mistakenly think we
    // are stalled.
...
void
UnloadDispenser(BATON_DISPOSITION batonDisposition) {
    StartRotorStallCheck();

    fRotorDispensingStopValid = false;
    float degStart = ReadAngleOnRotorStart();
    float degCaptureStop =
        degStart + DEG_ROTOR_TURN_DISP_TO_MAG + DEG_ROTOR_TURN_MAG_CARRY_BATON;
    StartRotor();
...
void
TurnRotorBy(ANGLE angle, int svposSpeed) {
    // Calculate an appropriate timeout value. This is VERY rough.  Note
    // also that the acheived rotor speed is HIGHLY non-linear in the
    // requested svpos value. More experimentation could give us a
    // mathematical model, but that work has not yet been done.
    ...
    MILLI msTimeout =
        Rounded((float)msFullRotation / 360.0 * abs(angle), MILLI);
