...
// An FTC configuration can only support at most 12 servos; we allow for
// them all here (uggh: RobotC seems to preclude more elegant
// approaches).
SERVO rgsvo[12];
...
// Function for testing whether a given servo position is valid for the
// given servo, etc
...
InitializeServo(IN OUT STICKYFAILURE &fOverallSuccess, SERVO &svo,
    ...
    // We require the main servo to be always there, but there may or
    // may not be a paired servo.
    if (svo.isvcMain < 0)
        fSuccess = false;
    ...
    svo.kind = kind;
    //
    const float radPhysicalServoRange =
        degreesToRadians(degPhysicalServoRange);
...
void
WaitForServos(MILLI msMinWait = 0, BOOL fBeep = true) {
    BOOL fSuccess = true;
    ...
    if (!USE_SERVO_CONTROLLER_WAITS) {
        ...
        // Actually ask the servo controllers themselves whether any
        // servos are still moving
        BOOL fAllComplete;
...
MoveServo(SERVO & svo, int svposNew, BOOL fMoveImmediately = false) {
    ...
    if (svo.fActive) {
        ...
        // external (unreversed) orientation
        ...
        // Figure out how long it will take to complete the command. Are
        // we raising or lowering the servo? That affects which
        // rate-scaling constant to use.  If we know the apogee, we
        // calculate up/down based thereon; if we don't then 'up' is
        // simply the direction of increasing svpos.
        int svposCur = GetInternalServoValue(svo);
        int dsvpos = svposNew - svposCur;
        MILLI msCommandDuration;

        if (SVOKIND_CNTROT == svo.kind) {
            // We don't actually need to wait much for continuous
            // rotation servos
            msCommandDuration = 10;     // a guess, needs testing
        } else if (!svo.fServoValueValid) {
            // We have to be entirely pessimistic, since we don't really
            // know where the servo is
            float msPerSvpos =
                svo.msPerSvpos * Max(svo.scaleDurUp, svo.scaleDurDown);
            ...
            // We calculate the rates for moving 'up' and 'down'
            // separately. The path to the apogee and the path after the
            // apogee have different rates.
            if ((dApogeeCur < 0 && dApogeeNew > 0) ||
                (dApogeeCur > 0 && dApogeeNew < 0)) {
                // The path passes *through* the apogee. The first part
                // of that path is 'up'; the second part is 'down'.
                float factorUp = svo.scaleDurUp * (float)Abs(dApogeeCur);
                float factorDown = svo.scaleDurDown * (float)Abs(dApogeeNew);
                msCommandDuration =
                    Rounded(svo.msPerSvpos * (factorUp + factorDown), long);
                ...
                float msPerSvpos =
                    svo.msPerSvpos * (fUp ? svo.scaleDurUp : svo.scaleDurDown);
                msCommandDuration =
                    Rounded((float)abs(dsvpos) * msPerSvpos, long);
        ...
        if (true) {
            // REVIEW: why doesn't the main 'fReversed' logic work
            // exactly like the fReflected stuff?
        ...
        if (HasPairedServo(svo)) {
            ...
            int svposPaired =
                (svo.fReflectedPaired ? svposLast - svposNew : svposNew);
        ...
        if (HasTrippedServo(svo)) {
            int svposTripped =
                (svo.fReflectedTripped ? svposLast - svposNew : svposNew);
        ...
        if (HasQuadServo(svo)) {
            int svposQuad =
                (svo.fReflectedQuad ? svposLast - svposNew : svposNew);
            svposQuad += svo.dsvposQuad;
            ClampVar(svposQuad, 0, svposLast);
            RecordSvoPosition(svo.isvoctlrQuad, svo.isvcQuad, svposQuad);
        }
        // If we've been asked to, then move the servos immediately
        // (otherwise we'll wait until a WaitForServos call to start
        // things going)
        if (fMoveImmediately) {
            LockDaisy();
            if (0 <= svo.isvoctlrMain)
                SendServoPositions(fSuccess, rgsvoctlr[svo.isvoctlrMain]);
            if (0 <= svo.isvoctlrPaired)
                SendServoPositions(fSuccess, rgsvoctlr[svo.isvoctlrPaired]);
            if (0 <= svo.isvoctlrTripped)
                SendServoPositions(fSuccess, rgsvoctlr[svo.isvoctlrTripped]);
            if (0 <= svo.isvoctlrQuad)
                SendServoPositions(fSuccess, rgsvoctlr[svo.isvoctlrQuad]);
            ReleaseDaisy();
        }
        // Having commanded that we go there, we can reasonably expect
        // that we know where the servo actually is in order to judge
        // future command durations.
