...
// Given a value of a type which structurally has BASECLASS as a prefix,
// cast the type of 'value' to be that of a BASECLASS&
...
//-----------------------------------------------------------------------------
// Some math. Implement as our own macros where needed so as to be
// task-insenstitive.
//-----------------------------------------------------------------------------
...
#define ClampVar(var,minVal,maxVal)                                     \
{                                                                       \
    if (var < (minVal)) {                                               \
        var = (minVal);                                                 \
    } else if (var > (maxVal)) {                                        \
...
#define ClampVarSign(var,minVal,maxVal)                                 \
{                                                                       \
    if (var < 0)                                                        \
        ClampVar(var,-(maxVal),-(minVal))                               \
    else                                                                \
        ClampVar(var,maxVal,minVal)                                     \
}

#if 0
    // This method of rounding produces magnitudes which are insensitive
    // to sign: 2.5 rounds to 3 just as -2.5 rounds to -3. This is
    // ideally what we want. However, we always end up evaluating the
    // argument x twice, and there's no way to avoid that in the macro
    // (and using a function isn't safely usable across tasks)
#    define Rounded(x,type)           ((x) < 0 ? ((type)((float)(x) - 0.5)) : ((type)((float)(x) + 0.5)))
#else
    // This method of rounding avoids the multiple evaluation of the
    // argument. The downside is that the exact-half-negatives round up
    // instead of down: -2.5 rounds to -2 (though -2.75 still rounds to
    // -3). We can live with the tradeoff. (Note that calling intrinsics
    // is task-safe.)
