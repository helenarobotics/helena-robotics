...
BOOL DoCommonJoystick(int jyc)
    ...
    //----------------------------------------------------------
        ...
        //
        // Idea: driver heads straight to goal, aligning approx close to 90deg head it. We
        //      * turn 90 deg to the left
        //      * drive forward a bit, then do DriveForwardsAndStraightenUsingRightSonic
        //      * call PositionUsingEOPD to position us at the edge of the dispenser
        //      * call DispenseFromEOPDPosition to actually carry out the dispensing.
        //
        ...
        if (255 != cmFromWallCur)
            {
            if (PositionUsingEOPD(disp, cmFromWallCur, RIGHT_SIDE))             // GetOverIt.h
                {
                if (DispenseFromEOPDPosition(disp, cmFromWallCur, STOP_BEFORE_DISPENSING))
                    {
...
BOOL DoDriveJoystick(int jyc)
    ...
    //----------------------------------------------------------
        ...
        {
        // Dump the preloads (might be useful if there's a baton stuck there somehow)
        //
        if (fPreloadArmExtended)
            {
            DumpPreload(true);
    ...
    //----------------------------------------------------------
        ...
        {
        // Set the arm swinging freely so that it won't get damaged by bots pushing on it
        DisableServoController(fSuccess, MainServoControllerOf(svoArmRotation));
        }
    ...
    else if (joyBtnOnce(jyc,1))  // Pack arm normally
        {
        PackArmSimply();
        WaitForServosOld(2000);
        }

    else if (joyBtnOnce(jyc,2))  // Pack arm from dead intermediate position
        {
    ...
    else if (joyBtnOnce(jyc,3))  // Pack arm from dead EOPD position
        {
        int cm = 23;
        DISPENSER disp = DISP_LOW;
        //
        int svposRotation, svposElbow, svposShoulder;
        LookupServoPositions(disp, cm, ARMPOS_DISPINT, svposRotation, svposElbow, svposShoulder);
        MoveDispenserArm(svposRotation, svposElbow, svposShoulder);
        WaitForServosOld(500);
        //
        LookupServoPositions(disp, cm, ARMPOS_EOPD, svposRotation, svposElbow, svposShoulder);
        MoveDispenserArm(svposRotation, svposElbow, svposShoulder);
        WaitForServosOld(500);
        //
        ...
        {
        // Toggle: deploy EOPD and position bot front-to-back / pack from EOPD
        //
...
void DoArmJoystick(int jyc)
    ...
    if (joyBtnOnce(jyc,JOYBTN_LEFTTRIGGER_UPPER))  // start / stop the dispenser rotating
        {
