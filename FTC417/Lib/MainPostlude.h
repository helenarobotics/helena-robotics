...
#define CM_BETWEEN_WHEELS               ((27.5 + 19.5) * 0.5)   // default: somewhere in the ballpark
#endif
...
const float crevMotorPerCm             = 1. / cmPerMotorRev;
const float cmPerEncoderTick           = cmPerMotorRev / (float)ENCODER_TICKS_PER_MOTOR_REV;
const float encoderTickPerCm           = 1. / cmPerEncoderTick;
...
BOOL InitializeMain(BOOL fBackgroundTasks, BOOL fInitArmServos)
    {
    ...
    // Try to deal with the 'one-time after long power off sonic sensors report incorrect types on muxes' bug
    if (!fOverallSuccess)
        {
        wait1Msec(500);          // No real reason, but it can only help.
        //
    ...
    if (fOverallSuccess)
        ...
        if (fBackgroundTasks)
            {
            // Start the blackboard task and make sure it updates at least once
