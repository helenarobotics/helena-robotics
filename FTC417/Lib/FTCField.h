...
// This UI is executed during the waitForStart() that one calls to await
// the start of a game round by the Field Control System (FCS). The UI
// herein, for debugging purposes, also allows waitForStart() to be
// manually forced by pressing the NXT orange button. The (a) and (b)
// parts of the UI are executed using the left and right gray arrows,
// while (c) is executed by pressing the gray button.
//
// Be sure to #include this file *before* you #include
// "joystickDriver.h" as it preempts some defaults that will be found
// therein.
//
// Note: the concept of 'left' vs 'right' side of the field was
// appropriate for the 2010-11 Get Over It competition, and may not be
// the most useful for later years. For now, the notion remains here in
// the code, but that may be adapted to something more useful in later
// games.
...
// Examine these variables in your code determine the output of this
// user interface

// are we to run teleop or autonomous mode once waitForStart returns?
PROGRAM_FLAVOR programFlavor = DEFAULT_PROGRAM_FLAVOR;

// are we playing as the red team or the blue team?
TEAM_COLOR teamColor = DEFAULT_TEAM_COLOR;

// are starting on the left or right side of the field?
STARTING_SIDE startingSide = DEFAULT_STARTING_SIDE;

// are we to use our 'demo' mode (if we have one) or not?
BOOL fUseProgramDemoMode = false;
...
// Define here in a simple way so that, at least, this file is self
// contained.  See also display.h, where a better sol'n is provided
// (wherein the message is in fact displayed only temporarily).
...
//
// Pressing the gray rectangular button will toggle the program demo
// mode (however, a left or right arrow must be pressed between each
// toggle, lest the program be terminated by the firmware).
//
...
{                                                                       \
    if (PROGRAM_FLAVOR_TELEOP == programFlavor) {                       \
        ...
        /* in 6/8 time! */                                              \
        PlayMusic(musicBluesRiff, 120, 3 * NOTE_EIGHTH);                \
    } else {                                                            \
...
{                                                                       \
    if (kExitButton == nNxtButtonPressed) {                             \
        ...
        if (PROGRAM_FLAVOR_AUTONOMOUS==programFlavor &&                 \
            TEAM_COLOR_RED==teamColor) {                                \
            ToggleStartingSide();                                       \
        } else {                                                        \
            if (PROGRAM_FLAVOR_TELEOP == programFlavor ||               \
                DEFAULT_PROGRAM_FLAVOR == PROGRAM_FLAVOR_AUTONOMOUS) {  \
