...
public class Region implements Serializable {
    ...
    // size of processed imaged (note that it may be downsampled from
    // original)
    int xPixels, yPixels;
    ...
    private void calculateEnclosingRectangle() {
        ...
        // debug
//        System.out.println(
//            "calculateEnclosingRectangle: x = " + enclosingRectangle.x +
//            ", y = " + enclosingRectangle.y +
//            ", h = " + enclosingRectangle.height +
//            ", w = " + enclosingRectangle.width);
    }


    // Returns area of bounding rectangle, measured in pixels^2.
    //
    // Note that we rely on 'get' function rather than accessing the
    // class value directly.  This accommodates possible 'lazy
    // instantiation' of that value, as enclosure calculations can be
    // relatively expensive, and may not be called unless necessary
     ...
     * Input: a thresholded grayscale image, ostensibly optimized to
     *   highlight pixels occupied with FRC reflective tape when
     *   illuminated with LED right.
     *
     * Applies a two-pass least mean squares line fit: the first pass
     *   fits to all leading points, the 2nd after outlier removal; a *
     *   qsort algorithm orders the points by resulting error, and a
     *   large * fraction (currently 1/2) of the data points - those
     *   with the * lowest fit errors -- are used for the 2nd pass.
     *   This makes it * much more robust to gaps, including those
     *   created at top, bottom, * and sides from loose fitting of
     *   enclosing rectangle from which * the algorithm begins.
     *
     *  Calcuates a set of four Point2D.Double's representing the four
     *    hoop corners, as visible (or extrapolated from visible edges)
     *
     *  Note that any pixel > 0 is assumed "on" -- a general grayscale
     *    scene would likely create disasterous results.
     */
        ...
        for (int y = r.y; y < r.y + r.height; y++) {
            for (int x = r.x; x < r.x + wsearch; x++) {
                if ((img.getRGB(x, y) &0x000000ff) > 0) {
                    // we only count edge cells that are off the image edge
                    if (x > 0) {
...
//        else System.out.println("Left count = " + count);
        ...
        // Calculate best line fit to right edge (using thresholded
        // image data)
        count = 0;
        for (int y = r.y; y < r.y + r.height; y++) {
...
//        else System.out.println("Top count = " + count);
        ...
        // Calculate best line fit to bottom edge (using thresholded
        // image data)
        count = 0;
        for (int x = r.x; x < r.x + r.width; x++) {
            int y;
...
*/
        // Save 4 lines that tightly bound the hoop, where possible.  We
        // set edge pointers to null where no line could be found (e.g.,
        // due to substantial occlusion or off camera FOV
...
/*
    static Point2D.Double intersectingPoint(double m1, double b1, double m2, double b2) {
        // x at point of intersection (if they intersect!)
...
*/
    ...
    /*
     * Two-pass least squares algorith.  First pass estimates a line,
     * sorts the results by residual error (on point-by-point basis),
    ...
    private double [] leastSquares(dataPoint dp [], int n, int keep) {
        ...
        // recalculate with 'keep' lowest error data points; return
        return leastSquares(dp, keep);
    }

    // Simple least squares linear fit algorithm.  Returns a 2-long
    // array of doubles, the first (slot [0]) of which represents m, 2nd
    // b, in the familiar line equation y = mx + b
...
//        System.out.println("y   = " + beta1 + " * x + " + beta0 + " std err " + Math.sqrt(err2/(double)n));
    ...
    private void quickSort(dataPoint arr[], int left, int right)  {
        //  watch out for  case of zero-length vector.  If this happes,
        //  partition blows up
        if (right <= left)
            return;
    ...
    /*
     * Logic to support organizing of detected regions ("hoops") into
     * hoop locations.
     */
    ...
    // Note again that y dimension is flipped; larger y's mean lower on
    // the image.
