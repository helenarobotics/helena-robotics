...
public class ThrottlePBarMWidget extends StaticWidget implements VarNotifier{
    ...
    public boolean validatePropertyChange(Property prop, Object value){
        if(prop == varProp && WidgetComm.typeFromName((String)value) != WidgetComm.DOUBLE)
            return false;
        return true;
    }

    public void varUpdate(){
        if(active && WidgetComm.getDoubleFromName(varProp.getValue()) != null){
            throttle = WidgetComm.getDoubleFromName(varProp.getValue());
            repaint();
        }
    }
    ...
    protected void paintComponent(Graphics g){
        if(active){
            ...
            g.fillRect(1,(int)((1-(throttle+1.0)/2)*((int)getSize().getHeight()-2)),(int)getSize().getWidth()-2,(int)((throttle+1)/2*((int)getSize().getHeight()-2)));
        }else{
