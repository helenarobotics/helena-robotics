...
void moveRobot(float right, float up);
...
task main()
{
    ...
    //Find the IR Beacon
    float firstAngle = findIRBeacon(1)+90;
    ...
    //MOVE ROBOT FW 2 FT
    moveRobot(MOVE_DIST);
    float secondAngle = findIRBeacon(1)+90;
    ...
    if (firstAngle < 90) {
        ...
        float right = cosDegrees(secondAngle) * r;
        float up = sinDegrees(secondAngle) * r;
        if (right > up) {
            //Move right right-up
        ...
        //Find how far to the right and how far up you have to move
        //NOTE: may want to use cosDegrees if using degrees
        float left = -cosDegrees(secondAngle) * r;
        float up = sinDegrees(secondAngle) * r;
